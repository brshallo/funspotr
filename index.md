...
[![CRAN
status](https://www.r-pkg.org/badges/version/funspotr)](https://CRAN.R-project.org/package=funspotr)
...
packages used in a few popular github repositories[¹](#fn1).
...
[blogdown](https://github.com/rstudio/blogdown) project[²](#fn2)).
Though see [Package dependencies in another
...
The primary function in funspotr is
[`spot_funs()`](https://brshallo.github.io/funspotr/reference/spot_funs.md)
which returns a dataframe showing the functions and associated packages
used in a file.
...
These can be combined with a variant of
[`spot_funs()`](https://brshallo.github.io/funspotr/reference/spot_funs.md)
that maps the function across each file path found,
...
- `spotted`:
  ...
  style list-column of results[⁴](#fn4) from mapping
  [`spot_funs()`](https://brshallo.github.io/funspotr/reference/spot_funs.md)
  across `absolute_paths`.

These results may then be unnested with the helper
[`funspotr::unnest_results()`](https://brshallo.github.io/funspotr/reference/unnest_results.md)
to provide a table of functions and packages by filepath. This can be
manipulated like any other dataframe – say we want to filter to only
those files where [here](https://here.r-lib.org/),
[readr](https://readr.tidyverse.org/) or
[rsample](https://rsample.tidymodels.org/) packages are used.
...
[`funspotr::network_plot()`](https://brshallo.github.io/funspotr/reference/network_plot.md)
to build a network visualization of the connections between
functions/packages and files[⁵](#fn5).
...
[`funspotr::check_pkgs_availability()`](https://brshallo.github.io/funspotr/reference/check_pkgs_availability.md)
to see which packages you are missing and install the missing packages
locally. If you don’t want to edit your global library you may want to
use [renv](https://rstudio.github.io/renv/) or other environment
management tools.

funspotr has an internal helper
[`funspotr::install_missing_pkgs()`](https://brshallo.github.io/funspotr/reference/install_missing_pkgs.md)
...
`renv::dependencies()` and only then start using funspotr[⁶](#fn6).
...
allows the user to explicitly specify `pkgs` where functions may come
from. This is useful in cases where the packages loaded are not in the
same location as the `file_path` (e.g. they are loaded via
[`source()`](https://rdrr.io/r/base/source.html), or a DESCRIPTION file,
or some other workflow). For example, below is a made-up example where
the [`library()`](https://rdrr.io/r/base/library.html) calls are made in
a separate file and [`source()`](https://rdrr.io/r/base/source.html)’d
in.
...
Also see
[`funspotr::spot_pkgs_from_description()`](https://brshallo.github.io/funspotr/reference/spot_pkgs_from_DESCRIPTION.md).
...
or
[`spot_funs_files()`](https://brshallo.github.io/funspotr/reference/spot_funs_files.md)
will return *all* instances of a function call rather than just once for
each file.

Compared to the initial example,
[`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html) now
shows-up at both rows 4 and 8:
...
you can add an inline function
[`funspotr::spot_tags()`](https://brshallo.github.io/funspotr/reference/spot_tags.md)
to a bullet in the `tags` or `categories` argument of your YAML header.
For example:
...
2.  Load packages. Explicit calls (e.g. `pkg::fun()`) are loaded
    ...
    are loaded last (putting them at the top of the search
    space)[⁷](#fn7).
...
3.  Pass file through
    ...
    and filter to just functions
4.  Pass functions through
    [`utils::find()`](https://rdrr.io/r/utils/apropos.html) to identify
    associated package

*Explainer slide from Rstudio Conf 2022
[presentation](https://www.youtube.com/watch?v=c9oU7ALJS3o):*

![](reference/figures/how-funspotr-works.png)
...
- funspotr is specific to R. If you try and pass in a file from a
  ...
  out[⁸](#fn8). The steps taken by funspotr would also not be needed in
  many other programming languages[⁹](#fn9).
- funspotr does not work perfectly at identifying functions or packages.
  ...
  [`switch()`](https://rdrr.io/r/base/switch.html). See
...
- [`knitr::read_chunk()`](https://rdrr.io/pkg/knitr/man/read_chunk.html)
  and [`knitr::purl()`](https://rdrr.io/pkg/knitr/man/knit.html) in a
  file passed to funspotr will also frequently cause an error in
  parsing. See [knitr#1753](https://github.com/yihui/knitr/issues/1753)
...
- As mentioned elsewhere, the default parsing of
  [`spot_funs()`](https://brshallo.github.io/funspotr/reference/spot_funs.md)
  is primarily for cases where package dependencies are loaded in the
  same file that they are used in[¹⁰](#fn10). Scripts that are not
...
- funspotr does not pay attention to when functions are reexported from
  ...
  functions it is simply looking at the top of the search
  space[¹¹](#fn11).
- Feel free to open an issue if you’d be interested in a simplifying
  function or vignette for mapping
  [`spot_funs()`](https://brshallo.github.io/funspotr/reference/spot_funs.md)
  through other folder structures not yet mentioned.
- All the functions in “R/spot-pkgs.R” would probably be better handled
  ...
  The simple regex’s I use have a variety of problems[¹²](#fn12).
...
- Rather than running
  ...
  it may make sense to instead clone the repo locally and then run
  [`list_files_wd()`](https://brshallo.github.io/funspotr/reference/list_files_wd.md)
  from the repo prior to running
  [`spot_funs_files()`](https://brshallo.github.io/funspotr/reference/spot_funs_files.md)
  as this will limit the number of API hits to github.
- Currently it’s possible to have github block you pretty soon due to
  ...
  number of github API hits (e.g. above bullet, or using
...
3.  Other arguments may produce additional columns. See
    [`spot_funs()`](https://brshallo.github.io/funspotr/reference/spot_funs.md)
    reference page for details.

4.  list-column output where each item is a list containing `result` and
    `error`.

5.  Took inspiration from
    [`plot()`](https://rdrr.io/r/graphics/plot.default.html) method in
    [cranly](https://github.com/ikosmidis/cranly).

6.  [renv](https://rstudio.github.io/renv/) is a more robust approach to
    finding and installing dependencies – particularly in cases where
...
7.  This heuristic is imperfect and means that a file with
    ...
    calls as coming from {MASS} – when what it should do is view the
    first was as coming from {dplyr} and the second from {MASS}.
...
10. i.e. in interactive R scripts or Rmd or qmd documents where you use
    [`library()`](https://rdrr.io/r/base/library.html) or related calls
    within the script.
...
12. e.g. in this case `lines <- "library(pkg)"` the `pkg` would show-up
    as a dependency despite just being part of a quote rather than
    actually loaded. See
