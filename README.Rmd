...
`spot_pkgs()` identifies packages loaded in an individual file and has an argument for `as_yaml_tags` that flattens the dependencies and outputs them in a format that can be pasted into the **tags** section of a blogdown post's YAML header. See ([blogdown#647](https://github.com/rstudio/blogdown/issues/647)).
