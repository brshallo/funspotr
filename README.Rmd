...
The goal of funspotr is to make it easy to identify which functions and packages are used in files. It was primarily written with a few popular github repositories in mind (see [brshallo/funspotr-examples](https://github.com/brshallo/funspotr-examples)).

funspotr is primarily designed for self-contained scripts (see [Package dependencies in another file]) and may not identify *every* function and/or package (see [Limitations, Problems, Musings]) or read the source code for details.
...
These results may then be unnested with the helper `funspotr::unnest_github_results()` to provide a table of which functions and packages are used. This can be manipulated like any other dataframe -- say we want to filter to those files where `{here}`, `{readr}` or `{rsample}` packages are used.
...
*To spot which package a function is from you must have the package installed locally.* Hence for files on other' github repos or that you created on a different machine, it is a good idea to start with `funspotr::check_pkgs_availability()` to see which packages you are missing.
...
* `knitr::read_chunk()` and `knitr::purl()` will often frequently error in parsing. See [knitr#1753](https://github.com/yihui/knitr/issues/1753) & [knitr#1938](https://github.com/yihui/knitr/issues/1938)
