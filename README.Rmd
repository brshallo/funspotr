...
* `list_files_*(): that identify files
...
funspotr has a few `list_files_*()` functions that return a dataframe of `relative_paths` and `absolute_paths` of all the R or R markdown files in different locations (e.g. github repo, gists). These can be combined with a variant of `spot_funs()` that maps the function across the paths, `spot_funs_files()`:
