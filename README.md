...
`{funspotr}` may not identify *every* function and/or package used in
scripts. See [Limitations, Problems,
...
loaded via `source()` or a DESCRIPTION file).
...
gh_repo_spotted <- github_spot_funs(
...
#>    <chr>   <chr>  <lgl>            <chr>                     <chr>              
#>  1 here    here   FALSE            R/Rmd-to-R.R              https://raw.github~
#>  2 purl    knitr  FALSE            R/Rmd-to-R.R              https://raw.github~
#>  3 filter  dplyr  TRUE             R/feat-engineering-lags.R https://raw.github~
#>  4 lag     dplyr  TRUE             R/feat-engineering-lags.R https://raw.github~
#>  5 filter  stats  TRUE             R/feat-engineering-lags.R https://raw.github~
#>  6 lag     stats  TRUE             R/feat-engineering-lags.R https://raw.github~
#>  7 glimpse tibble TRUE             R/feat-engineering-lags.R https://raw.github~
#>  8 glimpse dplyr  TRUE             R/feat-engineering-lags.R https://raw.github~
#>  9 one_of  tidyr  TRUE             R/feat-engineering-lags.R https://raw.github~
#> 10 one_of  dplyr  TRUE             R/feat-engineering-lags.R https://raw.github~
#> # ... with 118 more rows
```

You can set `preview = TRUE` in `github_spot_funs()` to first view the
files you intend to parse and spot functions in. (This is particularly
helpful in repositories with many files where it will take a while for
`github_spot_funs()` to run.)
...
for documentation) and only then start using `{funspotr}`.

(`{renv}` is a more robust approach than using the `{funspotr}`
functions and is particular important in cases where you are missing
many dependencies or don’t want to alter the packages in your global
library).
...
`spot_pkgs()` identifies packages loaded in an individual file and has
an argument for `as_yaml_tags` that flattens the dependencies and
outputs them in a format that can be pasted into the **tags** section of
a blogdown post’s YAML header. See
...
*If you have a suggestion for a function or set of functions for
spotting functions, feel free to open an issue.*

**If you’ve used {funspotr} to map the R functions and packages of a
public blog or repository, open an issue to add a link in the README.**

## Limitations, Problems, Musings

-   As mentioned elsewhere, the current parsing is primarily for cases
    where the packages are loaded in the same file that they are used in
    (i.e. in interactive scripts or notebooks). Scripts that are not
...
-   All the functions in “R/spot-pkgs.R” would be better handled by
    ...
    despite just being part of a quote rather than actually loading
    `pkg`.
-   The foundation of “R/spot-funs.R” is
    ...
    -   `NCmisc::list.functions.in.file()` does not properly consider
        ...
        -   Even when there is an explicit call to a function
            ...
            not know `fun` is coming from `pkg` specifically (if there
            is another package loaded elsewhere that also contains a
            function named `fun()`)
...
-   It may have made more sense to have `github_get_funs()` clone the
    repo locally rather than pointing to URL’s for each parsed file.
